...
/-! # Stipulated revenues and best responses -/
...
open Classical in
/-- The set of valid seller-revenue pairs `(r₁, r₂)` at prices `(p, q)` (\cref{thm:lne-cex:r}). -/
...
/-! Best-response revenues (\cref{thm:lne-cex:rstar}) -/
...
This works because we only ever compare `μ` against values `< cap`; see `Cap.lean`. -/

/-- The ratio r*/r with a fallback for when r=0. -/
noncomputable def ratio (fallback rstar r : ℝ) : ℝ :=
  if r = 0 then fallback else rstar / r

/-- Finite stand-in for `∞` inside `μ`. Used as `ratio`'s `fallback` inside `μ`.
Any value `≥ min_i μᵢ` that stays above every intermediate `μ(p,q) ≥ X` bound
in the case analysis works; `α·β + ν` gives ample headroom
...
with `x/0` read as `cap`. `(p,q)` is a `c`-NE iff `μ(p,q) ≤ c` (for `c < cap`);
minimizing `μ` over prices is the core optimization problem. -/
