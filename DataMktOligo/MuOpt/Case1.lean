...
/-- `r₂*(p) = ν` when `p ≥ α` (then `p/α ≥ 1`, so both `min(1,p/α) = 1` and
`max(1-p, 1) = 1`). -/
theorem r2star_eq_nu (h : Constraints α β ν) {p : ℝ} (hpα : α ≤ p) :
    r2star α ν p = ν := by
  have hα := alpha_pos h
...
theorem μ_eq_max_case1 (h : Constraints α β ν) {p q : ℝ}
    ...
    μ α β ν p q = max (ratio (cap α β ν) (r1star α β ν q) (r1lo β ν p q))
                      (ratio (cap α β ν) (r2star α ν p) (r2hi ν p q)) := by
  have hV : V α β ν p q = {(r1lo β ν p q, r2hi ν p q)} := by
...
chain `r₁⁻ ≤ β + ν(1-q₂) = q₂(αq₂+ν) ≤ q₂·r₁*(q)` gives `r₁*(q)/r₁⁻ ≥ 1/q₂ = μ₂`. -/
public theorem thm_1_1 (h : Constraints α β ν) {p q : ℝ}
    (hq : 0 ≤ q) (hpaq : α * q < p) (hpα : α ≤ p) :
    μ2 α β ν ≤ μ α β ν p q := by
  have hν := nu_pos h
  have hα := alpha_pos h
  have hpq1 : 1 < p + q := by linarith [h.c1_lo]
  rw [μ_eq_max_case1 h hq hpq1.le hpaq]
  rcases le_total q (q2 α β ν) with hqq2 | hq2q
  · -- `q ≤ q₂`: seller 2's ratio.
    refine le_trans ?_ (le_max_right _ _)
    rcases eq_or_lt_of_le hq with hq0 | hqpos
    · -- `q = 0`: `r₂⁺ = 0`, ratio is `cap ≥ μ₂`.
      have hr : r2hi ν p q = 0 := by
        rw [r2hi, ← hq0, min_eq_left (zero_le_one), mul_zero]
      rw [ratio, if_pos hr]
      exact μ2_le_cap h
    · -- `0 < q ≤ q₂ < 1`: ratio is `1/q ≥ 1/q₂ = μ₂`.
      have hq1 : q < 1 := lt_of_le_of_lt hqq2 (q2_lt_one h)
      rw [r2star_eq_nu h hpα, r2hi, min_eq_left (le_of_lt hq1),
          ratio, if_neg (mul_ne_zero (ne_of_gt hν) (ne_of_gt hqpos))]
      simp only [μ2]
      rw [div_le_div_iff₀ (q2_pos h) (mul_pos hν hqpos), one_mul]
      exact mul_le_mul_of_nonneg_left hqq2 hν.le
  · -- `q₂ ≤ q`: seller 1's ratio.
    ...
    have haq1 : (1:ℝ) ≤ α * q := by
      have : α * q2 α β ν ≤ α * q := mul_le_mul_of_nonneg_left hq2q hα.le
      linarith
    -- `r₁*(q) ≥ min(β,αq) + ν` (second `max` branch, since `αq ≥ 1`).
    have hM : min β (α * q) + ν ≤ r1star α β ν q := by
      simp only [r1star]
      rw [show min β (α * q) + ν * min 1 (α * q) = min β (α * q) + ν from by
            rw [min_eq_left haq1]; ring]
      exact le_max_right _ _
    -- `r₁⁻ ≤ β + ν·max(0, 1-q)`.
    have hr1lo_ub : r1lo β ν p q ≤ β + ν * max 0 (1 - q) := by
      simp only [r1lo]
      have h2 := mul_le_mul_of_nonneg_left (min_le_right p (max 0 (1 - q))) hν.le
      linarith [min_le_right p β, h2]
    -- `q₂(αq₂+ν) = β + ν(1-q₂)` (the defining quadratic of `q₂`).
    have hqid : q2 α β ν * (α * q2 α β ν + ν) = β + ν * (1 - q2 α β ν) := by
      linear_combination q2_quadratic h
    -- monotone chain: `β + ν·max(0,1-q) ≤ q₂·(min(β,αq) + ν)`.
    have hchain : β + ν * max 0 (1 - q) ≤ q2 α β ν * (min β (α * q) + ν) := by
      have hmaxle : max 0 (1 - q) ≤ 1 - q2 α β ν := by
        apply max_le
        · linarith [hq2lt]
        · linarith [hq2q]
      have hnmax : ν * max 0 (1 - q) ≤ ν * (1 - q2 α β ν) :=
        mul_le_mul_of_nonneg_left hmaxle hν.le
      have hminge : α * q2 α β ν ≤ min β (α * q) :=
        le_min hαq2β.le (mul_le_mul_of_nonneg_left hq2q hα.le)
      have hq2min : q2 α β ν * (α * q2 α β ν) ≤ q2 α β ν * min β (α * q) :=
        mul_le_mul_of_nonneg_left hminge (q2_pos h).le
      nlinarith [hqid, hnmax, hq2min]
    -- `r₁⁻ > 0`.
    have hr1lo_pos : 0 < r1lo β ν p q := by
      simp only [r1lo]
      have hm1 : α ≤ min p β := le_min (by linarith) h.c1_mid
      have hm2 : 0 ≤ ν * min p (max 0 (1 - q)) :=
        mul_nonneg hν.le (le_min (by linarith [h.c1_lo]) (le_max_left _ _))
      linarith [h.c1_lo]
    -- assemble: `μ₂ = 1/q₂ ≤ r₁*(q)/r₁⁻`.
    rw [ratio, if_neg (ne_of_gt hr1lo_pos)]
    simp only [μ2]
    rw [div_le_div_iff₀ (q2_pos h) hr1lo_pos, one_mul]
    have hstep : q2 α β ν * (min β (α * q) + ν) ≤ q2 α β ν * r1star α β ν q :=
...
clearing the denominator, `((ν+β) - ν·μ₃)·(αβ((ν+β)+ν·μ₃) + ν²·L₁) = β·(α(β+ν)² - βν²)`,
whose RHS is `> 0` by c4 and whose second factor is `> 0`, forcing `ν·μ₃ < ν+β`. -/
theorem thm_mu3_2 (h : Constraints α β ν) : μ3 α β ν < 1 + β / ν := by
  have hν := nu_pos h
  have hν0 : (ν:ℝ) ≠ 0 := hν.ne'
  have hα := alpha_pos h
  ...
  have hbr : 0 < α * β * ((ν + β) + ν * μ3 α β ν) + ν ^ 2 * L1 α β ν := by
    have h1 := mul_pos (mul_pos hα hβ)
      (by linarith [mul_pos hν hpos] : (0:ℝ) < (ν + β) + ν * μ3 α β ν)
    nlinarith [mul_pos (mul_pos hν hν) hL1]
  -- the c4 quantity is positive
  have hc4pos : 0 < β * (α * (β + ν) ^ 2 - β * ν ^ 2) := mul_pos hβ (by linarith [h.c4])
  -- the factorization identity (uses `μ₃`'s defining quadratic)
  have hfact : ((ν + β) - ν * μ3 α β ν) *
        (α * β * ((ν + β) + ν * μ3 α β ν) + ν ^ 2 * L1 α β ν)
      = β * (α * (β + ν) ^ 2 - β * ν ^ 2) := by
    simp only [L1, L2] at hquad ⊢
    linear_combination (-ν ^ 2) * hquad
  -- so the first factor is positive: `ν·μ₃ < ν+β`
  have hX : 0 < (ν + β) - ν * μ3 α β ν := by nlinarith [hfact, hbr, hc4pos]
  have hrw : 1 + β / ν - μ3 α β ν = ((ν + β) - ν * μ3 α β ν) / ν := by
    field_simp
  rw [← sub_pos, hrw]
  exact div_pos hX hν
...
two "slope" quantities `K := αμ₃² - νμ₃ + ν` and `M := α + νμ₃ - αμ₃²` and show each is
...
theorem mu3_K_pos (h : Constraints α β ν) :
  ...
  have hν := nu_pos h
  have hβ : 0 < β := by linarith [h.c1_lo, h.c1_mid]
  have hbn : 0 < β + ν - ν * μ3 α β ν := by
    have h1 := thm_mu3_2 h
    have h2 : ν * μ3 α β ν < ν * (1 + β / ν) := mul_lt_mul_of_pos_left h1 hν
    have h3 : ν * (1 + β / ν) = ν + β := by field_simp
    rw [h3] at h2; linarith
  have hE : β * (α * (μ3 α β ν) ^ 2 - ν * μ3 α β ν + ν)
      = (α + ν) * (β + ν - ν * μ3 α β ν) := by
...
theorem mu3_M_pos (h : Constraints α β ν) :
  ...
  have hM : β * (α + ν * μ3 α β ν - α * (μ3 α β ν) ^ 2)
      = (α + ν) * ν * (μ3 α β ν - 1) := by
    have hq := mu3_quadratic h
    simp only [L1, L2] at hq
...
`p < α·μ₃·q`, and seller 1's ratio dominates: writing `r₁⁻ = p + ν(1-q)` and using the
lower bounds `r₁*(q) ≥ β + ν(1-q)` (for `q ≤ q₁`) or `r₁*(q) ≥ αq + ν` (for `q ≥ q₁`),
the gap to `μ₃·r₁⁻` is `K·(β - q(α+ν))` resp. `M·(q(α+ν) - β)`, nonnegative by the sign
of `q - q₁`. -/
public theorem thm_1_2 (h : Constraints α β ν) {p q : ℝ}
    (hq : 0 ≤ q) (hpq1 : 1 ≤ p + q) (hpaq : α * q < p) (hpα : p ≤ α) :
    μ3 α β ν ≤ μ α β ν p q := by
  rw [μ_eq_max_case1 h hq hpq1 hpaq]
  rcases eq_or_lt_of_le hq with hq0 | hqpos
  · -- `q = 0`: `r₂⁺ = 0`, so `ratio₂ = cap ≥ μ₃`.
    have hr2cap : ratio (cap α β ν) (r2star α ν p) (r2hi ν p q) = cap α β ν := by
      have hr : r2hi ν p q = 0 := by rw [r2hi, ← hq0, min_eq_left zero_le_one, mul_zero]
      rw [ratio, if_pos hr]
    rw [hr2cap]
  ...
  have hr1lo : r1lo β ν p q = p + ν * (1 - q) := by
    simp only [r1lo]
    rw [min_eq_left (le_trans hpα h.c1_mid),
        max_eq_right (by linarith : (0:ℝ) ≤ 1 - q),
        min_eq_right (by linarith : 1 - q ≤ p)]
  have hr1lo_pos : 0 < r1lo β ν p q := by
    rw [hr1lo]; nlinarith [mul_pos hν (show (0:ℝ) < 1 - q by linarith)]
  -- seller 2's ratio is `p/(αq)`
  have hr2star : r2star α ν p = ν * (p / α) := by
  ...
  have hr2hi : r2hi ν p q = ν * q := by
    simp only [r2hi]; rw [min_eq_left (le_of_lt hqlt1)]
  have hratio2 : ratio (cap α β ν) (r2star α ν p) (r2hi ν p q) = p / (α * q) := by
    rw [hr2star, hr2hi, ratio, if_neg (mul_ne_zero (ne_of_gt hν) (ne_of_gt hqpos)),
        mul_div_mul_left (p / α) q (ne_of_gt hν), div_div]
  rcases le_total (α * μ3 α β ν * q) p with hA | hB
  · -- `p ≥ α·q·μ₃`: seller 2's ratio already clears `μ₃`.
    refine le_trans ?_ (le_max_right _ _)
    rw [hratio2, le_div_iff₀ (mul_pos hα hqpos)]
    nlinarith [hA]
  · -- `p ≤ α·μ₃·q`: seller 1's ratio clears `μ₃`.
    have hpB' : p ≤ α * μ3 α β ν * q := hB
    refine le_trans ?_ (le_max_left _ _)
    rw [ratio, if_neg (ne_of_gt hr1lo_pos), le_div_iff₀ hr1lo_pos, hr1lo]
    -- goal: `μ₃·(p + ν(1-q)) ≤ r₁*(q)`
    rcases le_total q (q1 α β ν) with hqq1 | hq1q
    · -- `q ≤ q₁`: use `r₁*(q) ≥ β + ν(1-q)`.
      have hg1 : β + ν * (1 - q) ≤ r1star α β ν q := by
        simp only [r1star]
        rw [max_eq_right (by linarith : (0:ℝ) ≤ 1 - q)]
        exact le_max_left _ _
      have hqle : q * (α + ν) ≤ β := by
        rw [q1, le_div_iff₀ (alpha_add_nu_pos h)] at hqq1; exact hqq1
      have hS1 : (α + ν) * ((β + ν * (1 - q)) -
            (α * (μ3 α β ν) ^ 2 * q + μ3 α β ν * ν * (1 - q)))
          = (α * (μ3 α β ν) ^ 2 - ν * μ3 α β ν + ν) * (β - q * (α + ν)) := by
        have hq := mu3_quadratic h
        simp only [L1, L2] at hq
        linear_combination (-1) * hq
      have hexpr : 0 ≤ (β + ν * (1 - q)) -
          (α * (μ3 α β ν) ^ 2 * q + μ3 α β ν * ν * (1 - q)) := by
        have hrhs : 0 ≤ (α * (μ3 α β ν) ^ 2 - ν * μ3 α β ν + ν) * (β - q * (α + ν)) :=
          mul_nonneg (mu3_K_pos h).le (by linarith [hqle])
        nlinarith [hS1, hrhs, alpha_add_nu_pos h]
      calc μ3 α β ν * (p + ν * (1 - q))
          ≤ μ3 α β ν * (α * μ3 α β ν * q + ν * (1 - q)) :=
            mul_le_mul_of_nonneg_left (by linarith [hpB']) hμ3.le
        _ = α * (μ3 α β ν) ^ 2 * q + μ3 α β ν * ν * (1 - q) := by ring
        _ ≤ β + ν * (1 - q) := by linarith [hexpr]
        _ ≤ r1star α β ν q := hg1
    · -- `q ≥ q₁`: use `r₁*(q) ≥ αq + ν`.
      have h1αq : 1 < α * q := by
      ...
      have hαqβ : α * q < β := by
        have h1 : α * q < α * 1 := by rw [mul_one]; linarith
        linarith [h.c1_mid]
      have hg2 : α * q + ν ≤ r1star α β ν q := by
        simp only [r1star]
        rw [min_eq_right (le_of_lt hαqβ), min_eq_left (le_of_lt h1αq), mul_one]
        exact le_max_right _ _
      have hqge : β ≤ q * (α + ν) := by
        rw [q1, div_le_iff₀ (alpha_add_nu_pos h)] at hq1q; exact hq1q
      have hS2 : (α + ν) * ((α * q + ν) -
            (α * (μ3 α β ν) ^ 2 * q + μ3 α β ν * ν * (1 - q)))
          = (α + ν * μ3 α β ν - α * (μ3 α β ν) ^ 2) * (q * (α + ν) - β) := by
        have hq := mu3_quadratic h
        simp only [L1, L2] at hq
        linear_combination (-1) * hq
      have hexpr2 : 0 ≤ (α * q + ν) -
          (α * (μ3 α β ν) ^ 2 * q + μ3 α β ν * ν * (1 - q)) := by
        have hrhs : 0 ≤ (α + ν * μ3 α β ν - α * (μ3 α β ν) ^ 2) * (q * (α + ν) - β) :=
          mul_nonneg (mu3_M_pos h).le (by linarith [hqge])
        nlinarith [hS2, hrhs, alpha_add_nu_pos h]
      calc μ3 α β ν * (p + ν * (1 - q))
          ≤ μ3 α β ν * (α * μ3 α β ν * q + ν * (1 - q)) :=
            mul_le_mul_of_nonneg_left (by linarith [hpB']) hμ3.le
        _ = α * (μ3 α β ν) ^ 2 * q + μ3 α β ν * ν * (1 - q) := by ring
        _ ≤ α * q + ν := by linarith [hexpr2]
        _ ≤ r1star α β ν q := hg2

/-- combination of thm_1_1 and thm_1_2 -/
public theorem thm_1 (h : Constraints α β ν) {p q : ℝ}
    (hq : 0 ≤ q) (hpq1 : 1 ≤ p + q) (hpaq : α * q < p) :
    min (μ2 α β ν) (μ3 α β ν) ≤ μ α β ν p q := by
