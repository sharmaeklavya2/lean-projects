...
theorem μ_eq_max_case3_raw {p q : ℝ} (hpq : p + q ≤ 1) :
    μ α β ν p q = max (ratio (cap α β ν) (r1star α β ν q) (r1lo β ν p q))
                      (ratio (cap α β ν) (r2star α ν p) (r2lo ν p q)) := by
  have hV : V α β ν p q = {(r1lo β ν p q, r2lo ν p q)} := by
...
`p ≤ p' = α/(α+1)`. Here `1 - p ≥ p/α`, so `r₁⁻ = p(ν+1)`, `r₂⁻ = nq`, and
`r₂*(p) = ν(1-p)`, giving `μ(p,q) = max(r₁*(q)/(p(ν+1)), (1-p)/q)`. -/
theorem μ_eq_max_case3 (h : Constraints α β ν) {p q : ℝ}
    (hp : 0 < p) (hq : 0 < q) (hpq : p + q ≤ 1) (hpp : p ≤ α / (α + 1)) :
    μ α β ν p q = max (r1star α β ν q / (p * (ν + 1))) ((1 - p) / q) := by
  have hα := alpha_pos h
  have hν := nu_pos h
  -- `p' = α/(α+1) < 1`, so `p < 1`.
  have hp1 : p < 1 :=
...
`μ(p,q) = max(r₁*(q)/(p(ν+1)), (1-p)/q)`, a `max` of two functions decreasing in `p`. -/
theorem mu_dec_p (h : Constraints α β ν) {q x1 x2 : ℝ}
    (hq0 : 0 ≤ q) (hx1 : 0 ≤ x1) (hx12 : x1 ≤ x2)
    (hx2 : x2 ≤ min (α / (α + 1)) (1 - q)) :
    min (cap α β ν) (μ α β ν x2 q) ≤ μ α β ν x1 q := by
  have hα := alpha_pos h
  have hν := nu_pos h
  ...
  · -- `x₁ = 0`: seller 1 earns `0`, so `μ(0,q) ≥ cap ≥ min cap (μ x₂ q)`.
    ...
    rw [show ratio (cap α β ν) (r1star α β ν q) 0 = cap α β ν from by rw [ratio, if_pos rfl]]
  ...
  · rcases eq_or_lt_of_le hq0 with hq00 | hqpos
    · -- `q = 0`: seller 2 earns `0`, so `μ(x₁,0) ≥ cap ≥ min cap (μ x₂ 0)`.
      ...
      rw [μ_eq_max_case3_raw (show x1 + (0:ℝ) ≤ 1 by linarith [hx1p',
            (by rw [div_lt_one (by linarith : (0:ℝ) < α + 1)]; linarith :
              α / (α + 1) < 1)]),
          hr2lo0]
      rw [show ratio (cap α β ν) (r2star α ν x1) 0 = cap α β ν from by rw [ratio, if_pos rfl]]
    ...
    · -- `0 < x₁ ≤ x₂` and `0 < q`: genuine monotonicity, both ratios finite.
      ...
      have hr1s_pos : 0 < r1star α β ν q := by
        have hlb : 0 < ν + α * q1 α β ν := by
          linarith [nu_pos h, one_lt_alpha_mul_q1 h]
      ...
      have hA : r1star α β ν q / (x2 * (ν + 1)) ≤ r1star α β ν q / (x1 * (ν + 1)) := by
...
theorem r1star_eq_of_le_q1 (h : Constraints α β ν) {q : ℝ}
  ...
  -- `g₂(q) ≤ g₂(q₁) = ν + α·q₁` (g₂ non-decreasing).
  have hg2mono : min β (α * q) + ν * min 1 (α * q) ≤ ν + α * q1 α β ν := by
    have hαle : α * q ≤ α * q1 α β ν := mul_le_mul_of_nonneg_left hqq1 hα.le
    have hm1 : min β (α * q) ≤ min β (α * q1 α β ν) := min_le_min le_rfl hαle
    have hm2 : min 1 (α * q) ≤ min 1 (α * q1 α β ν) := min_le_min le_rfl hαle
    linarith [hm1, mul_le_mul_of_nonneg_left hm2 hν.le, g2_q1 h]
  -- `ν + α·q₁ = g₁(q₁) = β + ν(1-q₁) ≤ β + ν(1-q)` (g₁ non-increasing).
  have hg1 : ν + α * q1 α β ν ≤ β + ν * (1 - q) := by
    have hgq1 := g1_q1 h
    rw [max_eq_right (by linarith [q1_lt_one h] : (0:ℝ) ≤ 1 - q1 α β ν)] at hgq1
    linarith [hgq1, mul_le_mul_of_nonneg_left (by linarith : 1 - q1 α β ν ≤ 1 - q) hν.le]
  unfold r1star
...
from them `μ(p,q) = max((β+ν(1-q))/(p(ν+1)), r₂*(p)/(nq))`, a `max` of two functions
decreasing in `q` (first numerator shrinks, second denominator grows). -/
theorem mu_dec_q (h : Constraints α β ν) {p y1 y2 : ℝ}
    (hp : 0 ≤ p) (hy1 : 0 ≤ y1) (hy12 : y1 ≤ y2)
    (hy2 : y2 ≤ min (q1 α β ν) (1 - p)) :
    min (cap α β ν) (μ α β ν p y2) ≤ μ α β ν p y1 := by
  have hα := alpha_pos h
  have hν := nu_pos h
  have hy2q1 : y2 ≤ q1 α β ν := le_trans hy2 (min_le_left _ _)
  have hy2p : y2 ≤ 1 - p := le_trans hy2 (min_le_right _ _)
  have hy1q1 : y1 ≤ q1 α β ν := le_trans hy12 hy2q1
  ...
  · rcases eq_or_lt_of_le hy1 with hy10 | hy1pos
    ...
    · -- `0 < p` and `0 < y₁ ≤ y₂`: genuine monotonicity, both ratios finite.
      ...
      -- seller-1 ratio: larger `q` ⇒ smaller numerator `β + ν(1-q)`.
      have hA : (β + ν * (1 - y2)) / (p * (ν + 1)) ≤ (β + ν * (1 - y1)) / (p * (ν + 1)) := by
        rw [div_le_div_iff₀ (mul_pos hppos (by linarith)) (mul_pos hppos (by linarith))]
        have hnum : β + ν * (1 - y2) ≤ β + ν * (1 - y1) :=
          by linarith [mul_le_mul_of_nonneg_left (by linarith : (1:ℝ) - y2 ≤ 1 - y1) hν.le]
        exact mul_le_mul_of_nonneg_right hnum (mul_pos hppos (by linarith)).le
      -- seller-2 ratio: larger `q` ⇒ larger denominator `nq`.
      have hB : r2star α ν p / (ν * y2) ≤ r2star α ν p / (ν * y1) := by
        rw [div_le_div_iff₀ (mul_pos hν hy2pos) (mul_pos hν hy1pos)]
        exact mul_le_mul_of_nonneg_left
...
theorem thm_pq_dom (h : Constraints α β ν) {p q : ℝ}
    (hp : 0 ≤ p) (hq : 0 ≤ q) (hpq : p + q ≤ 1) :
    ∃ ph qh : ℝ, ph + qh = 1 ∧ p ≤ ph ∧ q ≤ qh ∧
      min (cap α β ν) (μ α β ν ph qh) ≤ μ α β ν p q := by
  have hα := alpha_pos h
  ...
  have h1mpp : 1 - α / (α + 1) < q1 α β ν := by
    rw [hfrac]
  ...
  · rcases le_total (1 - q) (α / (α + 1)) with h1q | h1q
    ...
    · -- Case 2: `p ≤ p' ≤ 1-q`. Take `(p̂,q̂) = (p', 1-p')`; slide in both.
      ...
      have hA : min (cap α β ν) (μ α β ν (α / (α + 1)) q) ≤ μ α β ν p q :=
        mu_dec_p h hq hp hple (le_min le_rfl h1q)
      have hB : min (cap α β ν) (μ α β ν (α / (α + 1)) (1 - α / (α + 1)))
              ≤ μ α β ν (α / (α + 1)) q :=
...
public theorem thm_3 (h : Constraints α β ν) {p q : ℝ}
    (hp : 0 ≤ p) (hq : 0 ≤ q) (hpq : p + q ≤ 1) :
    min (μ1 α β ν) (min (μ2 α β ν) (μ3 α β ν)) ≤ μ α β ν p q := by
  ...
  have hBμ1 : min (μ1 α β ν) (min (μ2 α β ν) (μ3 α β ν)) ≤ μ1 α β ν := min_le_left _ _
  have hBμ2 : min (μ1 α β ν) (min (μ2 α β ν) (μ3 α β ν)) ≤ μ2 α β ν :=
    le_trans (min_le_right _ _) (min_le_left _ _)
  have hBμ3 : min (μ1 α β ν) (min (μ2 α β ν) (μ3 α β ν)) ≤ μ3 α β ν :=
    le_trans (min_le_right _ _) (min_le_right _ _)
  have hBcap : min (μ1 α β ν) (min (μ2 α β ν) (μ3 α β ν)) ≤ cap α β ν :=
    le_trans hBμ1 (μ1_le_cap h)
  -- `min(μ₁,μ₂,μ₃) ≤ μ(p̂,q̂)`.
  have hstar : min (μ1 α β ν) (min (μ2 α β ν) (μ3 α β ν)) ≤ μ α β ν ph qh := by
    ...
    · -- `p̂ = α·q̂`: the knife-edge point `(α/(α+1), 1/(α+1))`.
      ...
      have hph_le_p1 : ph ≤ p1 α β ν := by rw [hph_eq]; exact (p1_gt_ratio h).le
      -- `r₁*(q̂) ≥ (ν+1)·ĉ₁` (global bound, `= r₁*(q₁)`).
      have hr1 : (ν + 1) * chat1 α β ν ≤ r1star α β ν qh := by
        have h1 : (ν + 1) * chat1 α β ν = ν + α * q1 α β ν := by
          rw [← r1star_q1' h, r1star_q1 h]
        rw [h1]; exact r1star_ge h qh
      have hr1s_nonneg : 0 ≤ r1star α β ν qh :=
        le_trans (mul_pos (by linarith : (0:ℝ) < ν + 1)
          (by linarith [chat1_gt_one h] : (0:ℝ) < chat1 α β ν)).le hr1
      have hden : (0:ℝ) < ph * (ν + 1) := mul_pos hph_pos (by linarith)
      -- Seller 1's ratio clears `μ₁`: `ĉ₁/p₁ ≤ r₁*(q̂)/(p̂(ν+1))`.
      have hμ1_ineq : μ1 α β ν ≤ r1star α β ν qh / (ph * (ν + 1)) := by
        simp only [μ1]
      ...
      have hμ1 : μ1 α β ν ≤ μ α β ν ph qh := by
