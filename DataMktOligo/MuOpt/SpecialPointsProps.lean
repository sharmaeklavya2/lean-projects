...
public theorem alpha_pos (h : Constraints α β ν) : 0 < α := by linarith [h.c1_lo]

/-- `0 < ν`, from `α ≤ β < α + ν`. -/
public theorem nu_pos (h : Constraints α β ν) : 0 < ν := by linarith [h.c1_mid, h.c1_hi]
...
public theorem q1_lt_one (h : Constraints α β ν) : q1 α β ν < 1 := by
  simp only [q1]
  rw [div_lt_one (alpha_add_nu_pos h)]
  exact h.c1_hi

/-- `1 < α·q₁`, i.e. `q₁ > 1/α`, from Constraint c2 (`α + ν < α·β`). -/
public theorem one_lt_alpha_mul_q1 (h : Constraints α β ν) : 1 < α * q1 α β ν := by
  simp only [q1]
  rw [← mul_div_assoc, one_lt_div (alpha_add_nu_pos h)]
  exact h.c2

/-- `α·q₁ < β`, since `α·q₁ < α ≤ β` (uses `q₁ < 1`). -/
public theorem alpha_mul_q1_lt_beta (h : Constraints α β ν) : α * q1 α β ν < β := by
  have h1 : α * q1 α β ν < α * 1 := mul_lt_mul_of_pos_left (q1_lt_one h) (alpha_pos h)
  rw [mul_one] at h1
  linarith [h.c1_mid]

/-- First `max` branch of `r₁*` at `q₁`: `g₁(q₁) = β + ν·(1 - q₁) = ν + α·q₁`
(uses `q₁ < 1`). -/
public theorem g1_q1 (h : Constraints α β ν) :
    β + ν * max 0 (1 - q1 α β ν) = ν + α * q1 α β ν := by
  have hne : (α + ν) ≠ 0 := ne_of_gt (alpha_add_nu_pos h)
  rw [max_eq_right (by linarith [q1_lt_one h] : (0:ℝ) ≤ 1 - q1 α β ν)]
  simp only [q1]; field_simp; ring

/-- Second `max` branch of `r₁*` at `q₁`: `g₂(q₁) = α·q₁ + ν = ν + α·q₁`
(uses `1 < α·q₁ < β`). -/
public theorem g2_q1 (h : Constraints α β ν) :
    min β (α * q1 α β ν) + ν * min 1 (α * q1 α β ν) = ν + α * q1 α β ν := by
...
public theorem r1star_ge (h : Constraints α β ν) (q : ℝ) :
  ...
  unfold r1star
  rcases le_total q (q1 α β ν) with hle | hle
  · -- `q ≤ q₁`: `ν + α·q₁ = g₁(q₁) ≤ g₁(q) ≤ max`.
    refine le_trans ?_ (le_max_left _ _)
    rw [← g1_q1 h]
    have hmax : max 0 (1 - q1 α β ν) ≤ max 0 (1 - q) := max_le_max le_rfl (by linarith)
    have := mul_le_mul_of_nonneg_left hmax hν.le
    linarith
  · -- `q₁ ≤ q`: `ν + α·q₁ = g₂(q₁) ≤ g₂(q) ≤ max`.
    refine le_trans ?_ (le_max_right _ _)
    rw [← g2_q1 h]
    have hαle : α * q1 α β ν ≤ α * q := mul_le_mul_of_nonneg_left hle hα.le
    have hm1 : min β (α * q1 α β ν) ≤ min β (α * q) := min_le_min le_rfl hαle
    have hm2 : min 1 (α * q1 α β ν) ≤ min 1 (α * q) := min_le_min le_rfl hαle
...
theorem thm_q1 (h : Constraints α β ν) :
    ...
    ∀ q, 0 ≤ q → (ν + 1) * chat1 α β ν ≤ r1star α β ν q := by
  have hnc : (ν + 1) * chat1 α β ν = ν + α * q1 α β ν := by
...
public theorem alpha_mul_chat1_pos (h : Constraints α β ν) : 0 < α * chat1 α β ν :=
...
theorem p1_lt_one (h : Constraints α β ν) : p1 α β ν < 1 := by
  ...
  have hr1 : 1 < 1 + 4 / (α * chat1 α β ν) := by
    have : 0 < 4 / (α * chat1 α β ν) := by positivity
    linarith
  have hs_gt1 : 1 < Real.sqrt (1 + 4 / (α * chat1 α β ν)) := by
...
public theorem p1_quadratic (h : Constraints α β ν) :
  ...
  have hk : 0 < α * chat1 α β ν := alpha_mul_chat1_pos h
  simp only [p1]
  set k := α * chat1 α β ν with hk_def
...
theorem one_lt_p1_add_q1 (h : Constraints α β ν) : 1 < p1 α β ν + q1 α β ν := by
  ...
  have hα := alpha_pos h
  have hstep : α * (1 - p1 α β ν) < α * q1 α β ν := by linarith
...
/-- The `r₁⁺` component of `V` at `(p₁,q₁)`: `r₁⁺ = p₁·(1+ν)` (uses `p₁ < 1 ≤ β`). -/
theorem r1hi_p1_q1 (h : Constraints α β ν) :
    r1hi β ν (p1 α β ν) (q1 α β ν) = p1 α β ν * (1 + ν) := by
  have hp1_le_β : p1 α β ν ≤ β := by linarith [p1_lt_one h, h.c1_lo, h.c1_mid]
  simp only [r1hi]
  rw [min_eq_left hp1_le_β, min_eq_left (le_of_lt (p1_lt_one h))]
  ring

/-- The `r₂⁻` component of `V` at `(p₁,q₁)`: `r₂⁻ = ν·(1-p₁)` (uses `1-p₁ ≤ q₁`). -/
theorem r2lo_p1_q1 (h : Constraints α β ν) :
    r2lo ν (p1 α β ν) (q1 α β ν) = ν * (1 - p1 α β ν) := by
  have h1p : (0:ℝ) ≤ 1 - p1 α β ν := by linarith [p1_lt_one h]
  have hq1 : 1 - p1 α β ν ≤ q1 α β ν := by linarith [one_lt_p1_add_q1 h]
  simp only [r2lo]
  rw [max_eq_right h1p, min_eq_right hq1]

/-- `r₂*(p₁) = ν·(p₁/α)` (uses `1-p₁ ≤ p₁/α ≤ 1`). -/
theorem r2star_p1 (h : Constraints α β ν) :
    r2star α ν (p1 α β ν) = ν * (p1 α β ν / α) := by
  have hα := alpha_pos h
  have hp1_lt := p1_lt_one h
  have hp1_div_le : p1 α β ν / α ≤ 1 := by rw [div_le_one hα]; linarith [h.c1_lo]
  have h1p_le : 1 - p1 α β ν ≤ p1 α β ν / α := by
...
theorem ratio1_p1_q1 (h : Constraints α β ν) :
  ...
  have hp1_pos := p1_pos h
  have hν := nu_pos h
  have hden : p1 α β ν * (1 + ν) ≠ 0 :=
    mul_ne_zero (ne_of_gt hp1_pos) (ne_of_gt (by linarith : (0:ℝ) < 1 + ν))
...
theorem ratio2_p1_q1 (h : Constraints α β ν) :
  ...
  have hp1_pos := p1_pos h
  have hp1_lt := p1_lt_one h
  have h1p_pos : (0:ℝ) < 1 - p1 α β ν := by linarith
  have hden : ν * (1 - p1 α β ν) ≠ 0 := mul_ne_zero (ne_of_gt hν) (ne_of_gt h1p_pos)
  rw [r2star_p1 h, r2lo_p1_q1 h, ratio, if_neg hden,
      mul_div_mul_left (p1 α β ν / α) (1 - p1 α β ν) (ne_of_gt hν), div_div]
  simp only [μ1]
...
public theorem μ_p1_q1 (h : Constraints α β ν) :
  ...
  have hV : V α β ν (p1 α β ν) (q1 α β ν)
          = {(r1hi β ν (p1 α β ν) (q1 α β ν), r2lo ν (p1 α β ν) (q1 α β ν))} := by
    unfold V
    rw [if_neg (not_le.mpr (one_lt_p1_add_q1 h)), if_pos (p1_lt_alpha_mul_q1 h)]
  -- The `sInf`'s set is the singleton `{μ₁}` (both ratios equal `μ₁`).
  have hset : {m : ℝ | ∃ r1 r2, (r1, r2) ∈ V α β ν (p1 α β ν) (q1 α β ν) ∧
                m = max (ratio (cap α β ν) (r1star α β ν (q1 α β ν)) r1)
                        (ratio (cap α β ν) (r2star α ν (p1 α β ν)) r2)}
            = {μ1 α β ν} := by
    rw [hV]
    ext m
...
theorem thm_p1 (h : Constraints α β ν) :
    p1 α β ν ∈ Set.Ioo (α / (α + 1)) 1 ∧
    1 < chat1 α β ν ∧
    (p1 α β ν) ^ 2 = α * chat1 α β ν * (1 - p1 α β ν) ∧
...
public theorem L2_pos (h : Constraints α β ν) : 0 < L2 α β ν := by
  ...
  simp only [L2]
  nlinarith [mul_pos hα hν, mul_pos hα hβ, sq_nonneg ν]

/-- `0 < q₂`. -/
public theorem q2_pos (h : Constraints α β ν) : 0 < q2 α β ν := by
  have hβ : 0 < β := by linarith [h.c1_lo, h.c1_mid]
  have hν := nu_pos h
  simp only [q2]
...
theorem alpha_mul_q2 (h : Constraints α β ν) :
  ...
  have hν := nu_pos h
  have hs_pos : 0 ≤ Real.sqrt (L2 α β ν) := Real.sqrt_nonneg _
  have hs2 : Real.sqrt (L2 α β ν) ^ 2 = L2 α β ν := Real.sq_sqrt (L2_pos h).le
  simp only [q2]
  set s := Real.sqrt (L2 α β ν)
  have hden : (ν + s) ≠ 0 := ne_of_gt (by linarith)
...
public theorem q2_quadratic (h : Constraints α β ν) :
  ...
  have hq2 : α * q2 α β ν = Real.sqrt (L2 α β ν) - ν := alpha_mul_q2 h
  -- `(α·q₂ + ν)² = (√L₂)² = L₂`.
  have hsq : (α * q2 α β ν + ν) ^ 2 = L2 α β ν := by
    have he : α * q2 α β ν + ν = Real.sqrt (L2 α β ν) := by rw [hq2]; ring
    rw [he]; exact Real.sq_sqrt (L2_pos h).le
  have hL2 : L2 α β ν = ν ^ 2 + α * ν + α * β := by simp only [L2]
  -- `α·(α·q₂² + 2ν·q₂) = (α·q₂+ν)² - ν² = αν + αβ = α·(ν+β)`; cancel `α`.
  have hcancel : α * (α * q2 α β ν ^ 2 + 2 * ν * q2 α β ν) = α * (ν + β) := by
...
public theorem one_lt_alpha_mul_q2 (h : Constraints α β ν) : 1 < α * q2 α β ν := by
  have hν := nu_pos h
  rw [alpha_mul_q2 h]
  have h1 : ((ν + 1 : ℝ)) ^ 2 < L2 α β ν := by
    simp only [L2]
    nlinarith [mul_nonneg (show (0:ℝ) ≤ α - 2 by linarith [h.c1_lo]) hν.le,
      mul_nonneg (show (0:ℝ) ≤ α - 2 by linarith [h.c1_lo])
        (show (0:ℝ) ≤ β by linarith [h.c1_lo, h.c1_mid]), h.c1_lo, h.c1_mid]
  have h2 := Real.sqrt_lt_sqrt (sq_nonneg (ν + 1)) h1
  rw [Real.sqrt_sq (by linarith : (0:ℝ) ≤ ν + 1)] at h2
  linarith

/-- `1/α < q₂`, the lower end of `q₂ ∈ (1/α, 1)`. -/
theorem one_div_alpha_lt_q2 (h : Constraints α β ν) : 1 / α < q2 α β ν := by
  rw [div_lt_iff₀ (alpha_pos h)]
...
/-- The `r₁⁻` component of `V` at `(β,q₂)`: `r₁⁻ = β + ν·(1-q₂)`. -/
theorem r1lo_β_q2 (h : Constraints α β ν) :
    r1lo β ν β (q2 α β ν) = β + ν * (1 - q2 α β ν) := by
  have h1q : (0:ℝ) ≤ 1 - q2 α β ν := by linarith [q2_lt_one h]
  have h1q_le_β : 1 - q2 α β ν ≤ β := by linarith [q2_pos h, h.c1_lo, h.c1_mid]
  simp only [r1lo]
  rw [min_self, max_eq_right h1q, min_eq_right h1q_le_β]

/-- The `r₂⁺` component of `V` at `(β,q₂)`: `r₂⁺ = ν·q₂`. -/
theorem r2hi_β_q2 (h : Constraints α β ν) :
    r2hi ν β (q2 α β ν) = ν * q2 α β ν := by
  simp only [r2hi]
  rw [min_eq_left (le_of_lt (q2_lt_one h))]

/-- `r₁*(q₂) = max(β + ν(1-q₂), α·q₂ + ν)` (uses `1 < α·q₂ < β` and `q₂ < 1`). -/
theorem r1star_q2 (h : Constraints α β ν) :
    r1star α β ν (q2 α β ν) = max (β + ν * (1 - q2 α β ν)) (α * q2 α β ν + ν) := by
  have h1q : (0:ℝ) ≤ 1 - q2 α β ν := by linarith [q2_lt_one h]
  simp only [r1star]
  rw [max_eq_right h1q, min_eq_right (le_of_lt (alpha_mul_q2_lt_beta h)),
      min_eq_left (le_of_lt (one_lt_alpha_mul_q2 h)), mul_one]

/-- `r₂*(β) = ν` (uses `1 - β ≤ 1 ≤ β/α`). -/
theorem r2star_β (h : Constraints α β ν) : r2star α ν β = ν := by
  have hα := alpha_pos h
...
/-- Seller 2's ratio at `(β,q₂)` equals `μ₂`: `ν/(ν·q₂) = 1/q₂`. -/
theorem ratio2_β_q2 (h : Constraints α β ν) :
    ratio (cap α β ν) (r2star α ν β) (r2hi ν β (q2 α β ν)) = μ2 α β ν := by
  have hν := nu_pos h
  have hq2 := q2_pos h
  have hden : ν * q2 α β ν ≠ 0 := mul_ne_zero (ne_of_gt hν) (ne_of_gt hq2)
  rw [r2star_β h, r2hi_β_q2 h, ratio, if_neg hden]
  simp only [μ2]
  rw [div_eq_div_iff hden (ne_of_gt hq2)]
  ring

/-- Seller 1's ratio at `(β,q₂)` equals `μ₂`: with `A = β+ν(1-q₂)`, `B = α·q₂+ν`,
the quadratic gives `B·q₂ = A`, so `max(A,B) = B` (as `q₂ < 1`) and `B/A = 1/q₂`. -/
theorem ratio1_β_q2 (h : Constraints α β ν) :
    ratio (cap α β ν) (r1star α β ν (q2 α β ν)) (r1lo β ν β (q2 α β ν)) = μ2 α β ν := by
  have hν := nu_pos h
  have hq2 := q2_pos h
  have hq2lt := q2_lt_one h
  have hα := alpha_pos h
  have hA_pos : 0 < β + ν * (1 - q2 α β ν) := by
    have := mul_pos hν (show (0:ℝ) < 1 - q2 α β ν by linarith)
    linarith [h.c1_lo, h.c1_mid]
  have hB_pos : 0 < α * q2 α β ν + ν := by
    have := mul_pos hα hq2; linarith
  have hBA : (α * q2 α β ν + ν) * q2 α β ν = β + ν * (1 - q2 α β ν) := by
    linear_combination q2_quadratic h
  have hAB : β + ν * (1 - q2 α β ν) ≤ α * q2 α β ν + ν := by
...
public theorem μ_p2_q2 (h : Constraints α β ν) :
  ...
  have hV : V α β ν β (q2 α β ν)
          = {(r1lo β ν β (q2 α β ν), r2hi ν β (q2 α β ν))} := by
    unfold V
    rw [if_neg (not_le.mpr
          (by linarith [q2_pos h, h.c1_lo, h.c1_mid] : (1:ℝ) < β + q2 α β ν)),
        if_neg (not_lt.mpr (le_of_lt (alpha_mul_q2_lt_beta h))),
        if_pos (alpha_mul_q2_lt_beta h)]
  have hset : {m : ℝ | ∃ r1 r2, (r1, r2) ∈ V α β ν β (q2 α β ν) ∧
                m = max (ratio (cap α β ν) (r1star α β ν (q2 α β ν)) r1)
                        (ratio (cap α β ν) (r2star α ν β) r2)}
            = {μ2 α β ν} := by
    rw [hV]
    ext m
    simp only [Set.mem_ofPred_eq, Set.mem_singleton_iff, Prod.mk.injEq]
...
Not formalized: that `q₂` is the *unique* positive root of `(αx+ν)/(β+ν(1-x)) = 1/x`. -/
theorem thm_q2 (h : Constraints α β ν) :
    q2 α β ν ∈ Set.Ioo (1 / α) 1 ∧
...
/-- `0 < q₁ = β/(α+ν)`. -/
public theorem q1_pos (h : Constraints α β ν) : 0 < q1 α β ν :=
  div_pos (by linarith [h.c1_lo, h.c1_mid]) (alpha_add_nu_pos h)

/-- `0 < L₁ = α + ν - β`, from Constraint c1 (`β < α + ν`). -/
public theorem L1_pos (h : Constraints α β ν) : 0 < L1 α β ν := by
  simp only [L1]; linarith [h.c1_hi]

/-- The discriminant `D := ν²L₁² + 4αβL₂ ≥ 0` under `D`'s square root in `μ₃`. -/
theorem mu3_disc_nonneg (h : Constraints α β ν) :
    0 ≤ ν ^ 2 * (L1 α β ν) ^ 2 + 4 * α * β * L2 α β ν := by
  have hα := alpha_pos h
  have hβ : 0 < β := by linarith [h.c1_lo, h.c1_mid]
  have hL2 := L2_pos h
...
public theorem mu3_quadratic (h : Constraints α β ν) :
    α * β * (μ3 α β ν) ^ 2 + ν * L1 α β ν * μ3 α β ν - L2 α β ν = 0 := by
  have hα := alpha_pos h
  have hβ : 0 < β := by linarith [h.c1_lo, h.c1_mid]
  have hab : (2 * α * β : ℝ) ≠ 0 := ne_of_gt (by nlinarith [mul_pos hα hβ])
  set D := ν ^ 2 * (L1 α β ν) ^ 2 + 4 * α * β * L2 α β ν with hDdef
  have hD : 0 ≤ D := hDdef ▸ mu3_disc_nonneg h
  have hs2 : Real.sqrt D ^ 2 = D := Real.sq_sqrt hD
  have hval : 2 * α * β * μ3 α β ν + L1 α β ν * ν = Real.sqrt D := by
    rw [μ3, ← hDdef]; field_simp; ring
  have hsq : (2 * α * β * μ3 α β ν + L1 α β ν * ν) ^ 2 = D := by rw [hval]; exact hs2
  have key : 4 * α * β * (α * β * (μ3 α β ν) ^ 2 + ν * L1 α β ν * μ3 α β ν - L2 α β ν) = 0 := by
...
public theorem mu3_pos (h : Constraints α β ν) : 0 < μ3 α β ν := by
  ...
  have hL1 := L1_pos h
  have hL2 := L2_pos h
  have hLν_nonneg : 0 ≤ L1 α β ν * ν := mul_nonneg hL1.le hν.le
  have hlt : (L1 α β ν * ν) ^ 2 < ν ^ 2 * (L1 α β ν) ^ 2 + 4 * α * β * L2 α β ν := by
    nlinarith [mul_pos (mul_pos hα hβ) hL2]
  have h2 : Real.sqrt ((L1 α β ν * ν) ^ 2)
          < Real.sqrt (ν ^ 2 * (L1 α β ν) ^ 2 + 4 * α * β * L2 α β ν) :=
    Real.sqrt_lt_sqrt (by positivity) hlt
  rw [Real.sqrt_sq hLν_nonneg] at h2
  simp only [μ3]
...
public theorem one_lt_mu3 (h : Constraints α β ν) : 1 < μ3 α β ν := by
  ...
  have hbr : 0 < α * β * (μ3 α β ν + 1) + ν * L1 α β ν := by
    nlinarith [mul_pos hα hβ, mul_pos (mul_pos hα hβ) hpos, mul_pos hν hL1]
  have hfact : (μ3 α β ν - 1) * (α * β * (μ3 α β ν + 1) + ν * L1 α β ν) = β * ν := by
...
theorem beta_mul_mu3_lt (h : Constraints α β ν) : β * μ3 α β ν < α + ν := by
  ...
  have hquad := mu3_quadratic h
  have hbr : 0 < α * ((α + ν) + β * μ3 α β ν) + ν * L1 α β ν := by
    nlinarith [mul_pos hα hA, mul_pos (mul_pos hα hβ) hpos, mul_pos hν hL1]
  have hfact : ((α + ν) - β * μ3 α β ν) * (α * ((α + ν) + β * μ3 α β ν) + ν * L1 α β ν)
             = (α + ν) ^ 3 - β * (α * β + 2 * ν * (α + ν)) := by
...
theorem thm_mu3 (h : Constraints α β ν) :
    ...
    α * β * (μ3 α β ν) ^ 2 + ν * (ν + α - β) * (μ3 α β ν) - (ν ^ 2 + ν * α + α * β) = 0 := by
...
theorem p3_gt_alpha_q1 (h : Constraints α β ν) : α * q1 α β ν < p3 α β ν := by
  have hpos : 0 < α * q1 α β ν := mul_pos (alpha_pos h) (q1_pos h)
  simp only [p3]
  nlinarith [mul_pos hpos (sub_pos.mpr (one_lt_mu3 h))]

/-- `p₃ < α` (upper end of `p₃ ∈ (α·q₁, α)`): since `q₁·μ₃ < 1`. -/
theorem p3_lt_alpha (h : Constraints α β ν) : p3 α β ν < α := by
  have hα := alpha_pos h
  simp only [p3]
  nlinarith [mul_pos hα (sub_pos.mpr (q1_mul_mu3_lt_one h))]

/-- `1 < p₃`: since `1 < α·q₁ < p₃`. -/
theorem p3_gt_one (h : Constraints α β ν) : 1 < p3 α β ν :=
  lt_trans (one_lt_alpha_mul_q1 h) (p3_gt_alpha_q1 h)

/-- `p₃ < β`: since `p₃ < α ≤ β`. -/
theorem p3_lt_beta (h : Constraints α β ν) : p3 α β ν < β :=
  lt_of_lt_of_le (p3_lt_alpha h) h.c1_mid

/-- The `r₁⁻` component of `V` at `(p₃,q₁)`: `r₁⁻ = p₃ + ν·(1-q₁)`
(uses `p₃ < β` and `1-q₁ < 1 < p₃`). -/
theorem r1lo_p3_q1 (h : Constraints α β ν) :
    r1lo β ν (p3 α β ν) (q1 α β ν) = p3 α β ν + ν * (1 - q1 α β ν) := by
  have hq1 := q1_lt_one h
  have hq1p := q1_pos h
  have hp3 := p3_gt_one h
  simp only [r1lo]
  rw [min_eq_left (le_of_lt (p3_lt_beta h)),
      max_eq_right (by linarith : (0:ℝ) ≤ 1 - q1 α β ν),
      min_eq_right (by linarith : 1 - q1 α β ν ≤ p3 α β ν)]

/-- The `r₂⁺` component of `V` at `(p₃,q₁)`: `r₂⁺ = ν·q₁` (uses `q₁ < 1`). -/
theorem r2hi_p3_q1 (h : Constraints α β ν) :
    r2hi ν (p3 α β ν) (q1 α β ν) = ν * q1 α β ν := by
  simp only [r2hi]
  rw [min_eq_left (le_of_lt (q1_lt_one h))]

/-- `r₂*(p₃) = ν·(q₁·μ₃)` (uses `1-p₃ ≤ p₃/α ≤ 1` and `p₃/α = q₁·μ₃`). -/
theorem r2star_p3 (h : Constraints α β ν) :
    r2star α ν (p3 α β ν) = ν * (q1 α β ν * μ3 α β ν) := by
  have hα := alpha_pos h
  have hp3_1 := p3_gt_one h
  have hp3α : p3 α β ν / α = q1 α β ν * μ3 α β ν := by
    simp only [p3]; field_simp
  have hle1 : p3 α β ν / α ≤ 1 := by rw [hp3α]; linarith [q1_mul_mu3_lt_one h]
  have hle2 : 1 - p3 α β ν ≤ p3 α β ν / α := by
    have : 0 < p3 α β ν / α := div_pos (by linarith) hα
    linarith
  simp only [r2star]
  rw [min_eq_right hle1, max_eq_right hle2, hp3α]

/-- Seller 2's ratio at `(p₃,q₃)` equals `μ₃`: `ν·(q₁·μ₃)/(ν·q₁) = μ₃`. -/
theorem ratio2_p3_q1 (h : Constraints α β ν) :
    ratio (cap α β ν) (r2star α ν (p3 α β ν)) (r2hi ν (p3 α β ν) (q1 α β ν)) = μ3 α β ν := by
  have hν := nu_pos h
  have hq1 := q1_pos h
  have hνne : (ν : ℝ) ≠ 0 := ne_of_gt hν
  have hq1ne : q1 α β ν ≠ 0 := ne_of_gt hq1
  have hden : ν * q1 α β ν ≠ 0 := mul_ne_zero hνne hq1ne
  rw [r2star_p3 h, r2hi_p3_q1 h, ratio, if_neg hden]
  field_simp

/-- Seller 1's ratio at `(p₃,q₃)` equals `μ₃`: `(ν+α·q₁)/(p₃+ν(1-q₁)) = μ₃`.
Multiplying out `q₁ = β/(α+ν)` reduces to `L₂ = μ₃(αβμ₃+nL₁)`, i.e. the quadratic. -/
theorem ratio1_p3_q1 (h : Constraints α β ν) :
    ratio (cap α β ν) (r1star α β ν (q1 α β ν)) (r1lo β ν (p3 α β ν) (q1 α β ν)) = μ3 α β ν := by
  have hν := nu_pos h
  have hq1lt := q1_lt_one h
  have hp3 := p3_gt_one h
  have hAnne : (α + ν : ℝ) ≠ 0 := ne_of_gt (alpha_add_nu_pos h)
  have hquad := mu3_quadratic h
  have hden_pos : 0 < p3 α β ν + ν * (1 - q1 α β ν) := by
    have : 0 ≤ ν * (1 - q1 α β ν) := mul_nonneg hν.le (by linarith)
    linarith
...
public theorem μ_p3_q3 (h : Constraints α β ν) :
  ...
  have hV : V α β ν (p3 α β ν) (q1 α β ν)
          = {(r1lo β ν (p3 α β ν) (q1 α β ν), r2hi ν (p3 α β ν) (q1 α β ν))} := by
    unfold V
    rw [if_neg (not_le.mpr (by linarith [p3_gt_one h, q1_pos h] :
          (1:ℝ) < p3 α β ν + q1 α β ν)),
        if_neg (not_lt.mpr (le_of_lt (p3_gt_alpha_q1 h))),
        if_pos (p3_gt_alpha_q1 h)]
  have hset : {m : ℝ | ∃ r1 r2, (r1, r2) ∈ V α β ν (p3 α β ν) (q1 α β ν) ∧
                m = max (ratio (cap α β ν) (r1star α β ν (q1 α β ν)) r1)
                        (ratio (cap α β ν) (r2star α ν (p3 α β ν)) r2)}
            = {μ3 α β ν} := by
    rw [hV]
    ext m
    simp only [Set.mem_ofPred_eq, Set.mem_singleton_iff, Prod.mk.injEq]
...
theorem thm_p3 (h : Constraints α β ν) :
    p3 α β ν ∈ Set.Ioo (α * q3 α β ν) α ∧
