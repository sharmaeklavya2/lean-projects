...
With two datasets everything is explicit: there are only two consumption orders, and the
greedy bundle for either is given by two equations rather than a recursion. That is enough
to see that the demand correspondence is nonempty, which is what the pricing game needs. -/

/-- Some consumption order sorts two datasets by non-increasing bang-per-buck. -/
theorem exists_sorted_two {n : ℕ} (M : Instance n 2) (P : Fin 2 → ℝ) (I : Fin n) :
...
theorem exists_isGreedy_two {n : ℕ} (M : Instance n 2) (P : Fin 2 → ℝ) (I : Fin n)
...
theorem exists_demand_two {n : ℕ} (M : Instance n 2) (P : Fin 2 → ℝ) (I : Fin n)
