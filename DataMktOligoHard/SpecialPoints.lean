...
public theorem alpha_mul_chat1_pos (h : Constraints α β n) : 0 < α * chat1 α β n :=
...
/-- `0 < L₂ = n² + αn + αβ`. -/
public theorem L2_pos (h : Constraints α β n) : 0 < L2 α β n := by
  have hα := alpha_pos h
  have hn := n_pos h
...
public theorem mu3_lt_one_div_q1 (h : Constraints α β n) : μ3 α β n < 1 / q1 α β n := by
