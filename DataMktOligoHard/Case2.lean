...
theorem μ_eq_max_case2 (h : Constraints α β ν) {p q : ℝ}
    (hp : 0 ≤ p) (hpq1 : 1 ≤ p + q) (hpaq : p < α * q) :
    μ α β ν p q = max (ratio (cap α β ν) (r1star α β ν q) (r1hi β ν p q))
                      (ratio (cap α β ν) (r2star α ν p) (r2lo ν p q)) := by
  have hV : V α β ν p q = {(r1hi β ν p q, r2lo ν p q)} := by
    ...
    · -- boundary `p + q = 1`: the `{(r₁⁻, r₂⁻)}` branch, but `r₁⁻ = r₁⁺` here.
      ...
      have hr1 : r1lo β ν p q = r1hi β ν p q := by
  ...
  have hset : {m : ℝ | ∃ r1 r2, (r1, r2) ∈ V α β ν p q ∧
            ...
            = {max (ratio (cap α β ν) (r1star α β ν q) (r1hi β ν p q))
                   (ratio (cap α β ν) (r2star α ν p) (r2lo ν p q))} := by
    rw [hV]; ext m
...
for `p ≤ p₁` seller 1's ratio `r₁*(q)/(p(ν+1)) ≥ (ν+1)ĉ₁/(p(ν+1)) = ĉ₁/p ≥ ĉ₁/p₁ = μ₁`. -/
public theorem thm_2 (h : Constraints α β ν) {p q : ℝ}
    (hp : 0 ≤ p) (hq : 0 ≤ q) (hpaq : p < α * q) (hpq1 : 1 ≤ p + q) :
    μ1 α β ν ≤ μ α β ν p q := by
  have hν := nu_pos h
  have hα := alpha_pos h
  rw [μ_eq_max_case2 h hp hpq1 hpaq]
  rcases le_or_gt 1 p with hp1le | hplt1
  · -- `p ≥ 1`: seller 2 earns `0`, so `ratio₂ = cap ≥ min cap μ₁`.
    have hr2lo0 : r2lo ν p q = 0 := by
      simp only [r2lo]
      rw [max_eq_left (by linarith : (1:ℝ) - p ≤ 0), min_eq_right hq, mul_zero]
    have hratio2cap : ratio (cap α β ν) (r2star α ν p) (r2lo ν p q) = cap α β ν := by
  ...
  · -- `p < 1`.
    ...
    · -- `0 < p < 1`: the two genuine sub-cases.
      ...
      · -- `p ≥ p₁`: seller 2's ratio clears `μ₁`.
        ...
        have hden2 : r2lo ν p q ≠ 0 := by
          rw [hr2lo]; exact ne_of_gt (mul_pos hν h1p_pos)
        rw [ratio, if_neg hden2, hr2star, hr2lo,
            mul_div_mul_left _ _ (ne_of_gt hν), div_div]
        simp only [μ1]
