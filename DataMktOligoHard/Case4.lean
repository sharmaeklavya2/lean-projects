...
noncomputable def r1c (β ν p q z : ℝ) : ℝ := r1lo β ν p q + z * d β ν p q
...
noncomputable def μz (α β ν p q z : ℝ) : ℝ :=
  max (ratio (cap α β ν) (r1star α β ν q) (r1c β ν p q z))
      (ratio (cap α β ν) (r2star α ν p) (r2c β ν p q z))

/-- Paper `ẑ(q)`, the `z` where the two ratio curves cross. On the knife-edge
`p = α·q`, so we take `q` alone as the argument. -/
noncomputable def zhat (α β ν q : ℝ) : ℝ :=
  r2star α ν (α * q) * (r1star α β ν q - r1lo β ν (α * q) q)
    / ((r1star α β ν q + r2star α ν (α * q)) * d β ν (α * q) q)

/-- Paper `z*(q) = min(1, ẑ(q))`, the `z` minimizing `μ(q,z)`. -/
noncomputable def zstar (α β ν q : ℝ) : ℝ := min 1 (zhat α β ν q)
...
theorem r1lo_add_r2hi {p q : ℝ} (hp : 0 ≤ p) (hpq : 1 ≤ p + q) :
  ...
  have key : min p (max 0 (1 - q)) + min q 1 = 1 := by
    ...
    · rw [max_eq_right (by linarith), min_eq_right (by linarith), min_eq_left (by linarith)]
      ring
  have : ν * min p (max 0 (1 - q)) + ν * min q 1 = ν := by
...
theorem r1hi_add_r2lo {p q : ℝ} (hq : 0 ≤ q) (hpq : 1 ≤ p + q) :
  ...
  have key : min p 1 + min q (max 0 (1 - p)) = 1 := by
    ...
    · rw [max_eq_right (by linarith), min_eq_left (by linarith), min_eq_right (by linarith)]
      ring
  have : ν * min p 1 + ν * min q (max 0 (1 - p)) = ν := by
    rw [← mul_add, key, mul_one]
  linarith [this]
...
theorem μ_eq_inf_z (h : Constraints α β ν) {p q : ℝ}
  ...
  have hsum1 : r1lo β ν p q + r2hi ν p q = min p β + ν := by
    have h1 := r1lo_add_r2hi (β := β) (ν := ν) hppos.le hpq1.le; rwa [sSum] at h1
  have hsum2 : r1hi β ν p q + r2lo ν p q = min p β + ν := by
    have h2 := r1hi_add_r2lo (β := β) (ν := ν) hqpos.le hpq1.le; rwa [sSum] at h2
  have hdpos : 0 < d β ν p q := d_pos (nu_pos h) hppos hqpos hpq1
  have hdne : d β ν p q ≠ 0 := ne_of_gt hdpos
  have hd_eq : d β ν p q = r1hi β ν p q - r1lo β ν p q := rfl
  have hr2lo_eq : r2lo ν p q = r2hi ν p q - d β ν p q := by unfold d; linarith [hsum1, hsum2]
  -- V is exactly the interpolating segment
  have hV : V α β ν p q = {rr : ℝ × ℝ | r1lo β ν p q ≤ rr.1 ∧ rr.1 ≤ r1hi β ν p q ∧
      r2lo ν p q ≤ rr.2 ∧ rr.2 ≤ r2hi ν p q ∧ rr.1 + rr.2 = min p β + ν} := by
    unfold V
  ...
  · rintro ⟨r1, r2, hmem, hm⟩
    ...
    refine ⟨(r1 - r1lo β ν p q) / (d β ν p q), ⟨?_, ?_⟩, ?_⟩
    · exact div_nonneg (by linarith) hdpos.le
    · exact (div_le_one hdpos).mpr (by rw [hd_eq]; linarith)
    · have hz1 : r1c β ν p q ((r1 - r1lo β ν p q) / (d β ν p q)) = r1 := by
        rw [r1c, div_mul_cancel₀ _ hdne]; ring
      have hz2 : r2c β ν p q ((r1 - r1lo β ν p q) / (d β ν p q)) = r2 := by
        rw [r2c, div_mul_cancel₀ _ hdne]; linarith [hsum, hsum1]
      rw [μz, hz1, hz2]; exact hm.symm
  · rintro ⟨z, ⟨hz0, hz1u⟩, hm⟩
    have hzd_le : z * d β ν p q ≤ d β ν p q := by
      nlinarith [mul_nonneg (by linarith : (0:ℝ) ≤ 1 - z) hdpos.le]
    have hzd_nonneg : 0 ≤ z * d β ν p q := mul_nonneg hz0 hdpos.le
    refine ⟨r1c β ν p q z, r2c β ν p q z, ?_, ?_⟩
    · rw [hV]
...
theorem knife_r2hi_eq (h : Constraints α β ν) {p q : ℝ} (hpaq : p = α * q)
    (hqlb : 1 / (α + 1) < q) : r2hi ν p q = r2star α ν p := by
...
/-- Plain-variable arithmetic for the `cap` corner: `(A+B)/s ≤ α·β + ν`
given `A ≤ β+ν`, `B ≤ ν`, `s ≥ 1+ν`, and the constraints `α ≥ 2, β,ν > 0`. -/
private lemma cap_corner_bound {A B ν α β s : ℝ} (hA : A ≤ β + ν) (hB : B ≤ ν)
    (hs : 1 + ν ≤ s) (hspos : 0 < s) (hα : 2 ≤ α) (hβ : 2 ≤ β) (hν : 0 < ν) :
    (A + B) / s ≤ α * β + ν := by
  have hcap_pos : (0:ℝ) < α * β + ν := by nlinarith
  rw [div_le_iff₀ hspos]
...
theorem thm_4_1 (h : Constraints α β ν) {p q : ℝ}
    ...
    ∧ μz α β ν p q (zstar α β ν q)
      = max ((r1star α β ν q + r2star α ν p) / sSum β ν p)
            (r1star α β ν q / r1hi β ν p q) := by
  -- abbreviations for the paper's quantities
  set A := r1star α β ν q with hAdef
  set B := r2star α ν p with hBdef
  set Rlo := r1lo β ν p q with hRlodef
  set Rhi := r1hi β ν p q with hRhidef
  set D := d β ν p q with hDdef
  set s := sSum β ν p with hsdef
  -- positivity on the knife-edge
  have hα0 : (0:ℝ) < α := by linarith [h.c1_lo]
  have hα1 : (0:ℝ) < α + 1 := by linarith [h.c1_lo]
  have hβ0 : (0:ℝ) < β := by linarith [h.c1_lo, h.c1_mid]
  have hν0 : (0:ℝ) < ν := nu_pos h
  have hqlb : 1 / (α + 1) < q := q_lb h (hpaq ▸ hpq1)
  have hqpos : 0 < q := lt_trans (div_pos one_pos hα1) hqlb
  have hppos : 0 < p := by rw [hpaq]; exact mul_pos hα0 hqpos
  -- knife-edge observations
  have hr2 : r2hi ν p q = B := knife_r2hi_eq h hpaq hqlb
  have hAR : Rhi ≤ A := knife_r1hi_le hpaq
  have hDpos : 0 < D := d_pos hν0 hppos hqpos hpq1
  have hsum1 : Rlo + r2hi ν p q = min p β + ν := by
    have h1 := r1lo_add_r2hi (β := β) (ν := ν) hppos.le hpq1.le; rwa [sSum] at h1
  have hRlopos : 0 < Rlo := by
    rw [hRlodef, r1lo]
    nlinarith [lt_min hppos hβ0,
      mul_nonneg hν0.le (le_min hppos.le (le_max_left (0:ℝ) (1 - q)))]
  have hBnn : 0 ≤ B := by rw [← hr2, r2hi]; exact mul_nonneg hν0.le (le_min hqpos.le (by norm_num))
  have hRlo_le_Rhi : Rlo ≤ Rhi := by
    rw [hRlodef, hRhidef, r1lo, r1hi]
    have hmx : max 0 (1 - q) ≤ 1 := max_le (by norm_num) (by linarith [hqpos])
    nlinarith [mul_le_mul_of_nonneg_left (min_le_min (le_refl p) hmx) hν0.le]
  ...
  have hr2lo_eq : r2lo ν p q = r2hi ν p q - D := by
    have h2 := r1hi_add_r2lo (β := β) (ν := ν) hqpos.le hpq1.le
    rw [sSum] at h2; rw [hDdef, d]; linarith [hsum1]
  have hRhi_eq : Rhi = Rlo + D := by rw [hRhidef, hRlodef, hDdef, d]; ring
  have hr1c_fold : ∀ z, r1c β ν p q z = Rlo + z * D := by
    intro z; rw [r1c, ← hRlodef, ← hDdef]
  have hr2c_fold : ∀ z, r2c β ν p q z = B - z * D := by
    intro z; rw [r2c, ← hDdef, hr2]
  have hr2lo0 : 0 ≤ r2lo ν p q := mul_nonneg hν0.le (le_min hqpos.le (le_max_left _ _))
  have hr2lo_BD : r2lo ν p q = B - D := by rw [hr2lo_eq, hr2]
  have hDB : D ≤ B := by rw [hr2lo_BD] at hr2lo0; linarith
  have hr2c_nonneg : ∀ z, z ≤ 1 → 0 ≤ r2c β ν p q z := by
    intro z hz1
    rw [hr2c_fold z]
    nlinarith [mul_le_of_le_one_left hDpos.le hz1, hDB]
  have hr1c_pos : ∀ z, 0 ≤ z → 0 < r1c β ν p q z := by
    intro z hz0
    rw [hr1c_fold z]; nlinarith [mul_nonneg hz0 hDpos.le, hRlopos]
  have hμz_eq : ∀ z, μz α β ν p q z
      = max (ratio (cap α β ν) A (r1c β ν p q z)) (ratio (cap α β ν) B (r2c β ν p q z)) := by
  ...
  have hA_ub : A ≤ β + ν := by
    ...
    · nlinarith [mul_le_mul_of_nonneg_left
        (max_le (by norm_num) (by linarith [hqpos]) : max 0 (1 - q) ≤ 1) hν0.le]
    · nlinarith [mul_le_mul_of_nonneg_left (min_le_left 1 (α * q)) hν0.le, min_le_left β (α * q)]
  have hB_ub : B ≤ ν := by
    rw [← hr2, r2hi]
    nlinarith [mul_le_mul_of_nonneg_left (min_le_right q 1) hν0.le]
  -- zstar ∈ [0,1]
  have hzhat_eq : zhat α β ν q = B * (A - Rlo) / ((A + B) * D) := by
    rw [zhat, ← hpaq, ← hAdef, ← hBdef, ← hRlodef, ← hDdef]
  have hzhat_nonneg : 0 ≤ zhat α β ν q := by
    rw [hzhat_eq]
    apply div_nonneg (mul_nonneg hBnn (by linarith [hAR, hRlo_le_Rhi]))
    exact mul_nonneg (by linarith [hApos, hBnn]) hDpos.le
  have hzstar_mem : zstar α β ν q ∈ Set.Icc (0:ℝ) 1 := by
  ...
  have hBpos : 0 < B := by rw [← hr2, r2hi]; exact mul_pos hν0 (lt_min hqpos one_pos)
  have hVal : μz α β ν p q (zstar α β ν q)
      = max ((A + B) / s) (A / Rhi) := by
    have hzstar_eq : zstar α β ν q = min 1 (B * (A - Rlo) / ((A + B) * D)) := by
      rw [zstar, hzhat_eq]
    ...
    rw [hμz_eq (zstar α β ν q), hr1c_fold, hr2c_fold, hzstar_eq, ratio, if_neg hr1pos.ne']
  ...
  have hcorner : ∀ z, 0 ≤ z → z ≤ 1 → B - z * D = 0 → (A + B) / s ≤ cap α β ν := by
    intro z hz0 hz1 h0
    have hBleD : B ≤ D := by nlinarith only [mul_le_of_le_one_left hDpos.le hz1, h0]
    have hr2lo_zero : r2lo ν p q = 0 := by
    ...
    exact cap_corner_bound hA_ub hB_ub hs_lb hspos h.c1_lo
      (by linarith [h.c1_lo, h.c1_mid]) hν0
  have hLBval : ∀ z ∈ Set.Icc (0:ℝ) 1, max ((A + B) / s) (A / Rhi) ≤ μz α β ν p q z := by
    intro z hz
    obtain ⟨hz0, hz1⟩ := hz
    have hr1cpos := hr1c_pos z hz0
    have hratio1 : ratio (cap α β ν) A (r1c β ν p q z) = A / r1c β ν p q z := by
  ...
  -- assembly
  refine ⟨?_, hVal⟩
  have hLeast : IsLeast (Set.image (μz α β ν p q) (Set.Icc 0 1))
      (μz α β ν p q (zstar α β ν q)) := by
    constructor
    · exact ⟨zstar α β ν q, hzstar_mem, rfl⟩
...
theorem thm_mu4 (h : Constraints α β ν) :
  ...
  have hpaq : p4 α β ν = α * q4 α β ν := by rw [p4, q4]
  have hp0 : 0 ≤ p4 α β ν := by rw [p4]; exact (mul_pos hα hq1pos).le
  have hq0 : 0 ≤ q4 α β ν := by rw [q4]; exact hq1pos.le
  have hpq1 : 1 < p4 α β ν + q4 α β ν := by
    rw [p4, q4]; nlinarith [one_lt_alpha_mul_q1 h, hq1pos]
  rw [(thm_4_1 h hpaq hpq1).2, p4, q4]
  -- the four revenue values at `(α·q₁, q₁)`
  have hA : r1star α β ν (q1 α β ν) = ν + α * q1 α β ν := r1star_q1 h
  have hB : r2star α ν (α * q1 α β ν) = ν * q1 α β ν := by
    rw [r2star]
    have hdiv : (α * q1 α β ν) / α = q1 α β ν := by
  ...
  have hRhi : r1hi β ν (α * q1 α β ν) (q1 α β ν) = α * q1 α β ν + ν := by
    rw [r1hi, min_eq_left (alpha_mul_q1_lt_beta h).le,
        min_eq_right (one_lt_alpha_mul_q1 h).le, mul_one]
  have hS : sSum β ν (α * q1 α β ν) = α * q1 α β ν + ν := by
    rw [sSum, min_eq_left (alpha_mul_q1_lt_beta h).le]
  rw [hA, hB, hRhi, hS]
  have hden : 0 < α * q1 α β ν + ν := by nlinarith [mul_pos hα hq1pos]
  have h2nd : (ν + α * q1 α β ν) / (α * q1 α β ν + ν) = 1 := by
    rw [div_eq_one_iff_eq (ne_of_gt hden)]; ring
  have hαν : (α + ν) ≠ 0 := ne_of_gt (alpha_add_nu_pos h)
  have hL2ne : ν ^ 2 + α * ν + α * β ≠ 0 := by positivity
  have hX : (ν + α * q1 α β ν + ν * q1 α β ν) / (α * q1 α β ν + ν) = μ4 α β ν := by
    rw [μ4, L2, q1]; field_simp; ring
  have hμ4ge : (1:ℝ) ≤ μ4 α β ν := by
    rw [μ4]; have : 0 ≤ β * ν / L2 α β ν := by rw [L2]; positivity
    linarith
  rw [h2nd, hX, max_eq_left hμ4ge]

/-- `μ(p₄, q₄) = μ₄` -/
public theorem μ_p4_q4 (h : Constraints α β ν) :
    μ α β ν (p4 α β ν) (q4 α β ν) = μ4 α β ν := by
  have hpaq : p4 α β ν = α * q4 α β ν := by rw [p4, q4]
  have hpq1 : 1 < p4 α β ν + q4 α β ν := by
...
theorem thm_4_2 (h : Constraints α β ν) {p q : ℝ}
    (hpaq : p = α * q) (hpq1 : 1 < p + q) :
    min (μ2 α β ν) (μ4 α β ν) ≤ μz α β ν p q (zstar α β ν q) := by
  have hα := alpha_pos h
  have hν := nu_pos h
  have hβ : (0:ℝ) < β := by linarith [h.c1_lo, h.c1_mid]
  have hα1 : (0:ℝ) < α + 1 := by linarith [h.c1_lo]
  ...
  have hqpos : 0 < q := lt_trans (div_pos one_pos hα1) hqlb
  have hq1pos := q1_pos h
  have hr2 : r2star α ν p = ν * min q 1 := by rw [← knife_r2hi_eq h hpaq hqlb, r2hi]
  -- reduce to `(r1* + r2*) / s`
  rw [(thm_4_1 h hpaq hpq1).2]
  refine le_trans ?_ (le_max_left _ _)
  have hsSum : sSum β ν p = min β (α * q) + ν := by rw [sSum, hpaq, min_comm]
  rcases le_total q 1 with hqle1 | hqge1
  · -- q ≤ 1 : bound below by μ₄
    refine le_trans (min_le_right _ _) ?_
    have hr2v : r2star α ν p = ν * q := by rw [hr2, min_eq_left hqle1]
    have hαq_le_α : α * q ≤ α := by nlinarith [mul_le_mul_of_nonneg_left hqle1 hα.le]
    have hαqβ : α * q ≤ β := by linarith [h.c1_mid]
    have hs : sSum β ν p = α * q + ν := by rw [hsSum, min_eq_right hαqβ]
    have hden_r : 0 < α * q + ν := by nlinarith [mul_pos hα hqpos]
    have hden_l : 0 < α * q1 α β ν + ν := by nlinarith [mul_pos hα hq1pos]
    rcases le_total q (q1 α β ν) with hqq1 | hqq1
    · -- q ≤ q₁ : (β+ν)/(αq+ν), decreasing
      have hμ4 : μ4 α β ν = (β + ν) / (α * q1 α β ν + ν) := by
        rw [μ4, L2, q1]; field_simp; ring
      have hr1lb : β + ν - ν * q ≤ r1star α β ν q := by
        rw [r1star]; refine le_trans ?_ (le_max_left _ _)
        nlinarith [mul_le_mul_of_nonneg_left (le_max_right 0 (1 - q)) hν.le]
      rw [hμ4, hs, hr2v, div_le_div_iff₀ hden_l hden_r]
      nlinarith [mul_nonneg (by linarith [hr1lb] : (0:ℝ) ≤ r1star α β ν q + ν * q - (β + ν))
          hden_l.le,
        mul_nonneg (by linarith : (0:ℝ) ≤ β + ν)
          (by nlinarith [mul_le_mul_of_nonneg_left hqq1 hα.le] :
            (0:ℝ) ≤ α * q1 α β ν + ν - (α * q + ν))]
    · -- q₁ ≤ q ≤ 1 : 1 + nq/(αq+ν), increasing
      have h1αq : 1 ≤ α * q := by
        nlinarith [one_lt_alpha_mul_q1 h, mul_le_mul_of_nonneg_left hqq1 hα.le]
      have hr1lb2 : α * q + ν ≤ r1star α β ν q := by
        rw [r1star]; refine le_trans (le_of_eq ?_) (le_max_right _ _)
        rw [min_eq_right hαqβ, min_eq_left h1αq, mul_one]
      have hμ4c : μ4 α β ν = 1 + ν * q1 α β ν / (α * q1 α β ν + ν) := by
        rw [μ4, L2, q1]; field_simp; ring
      have step : 1 + ν * q / (α * q + ν) ≤ (r1star α β ν q + ν * q) / (α * q + ν) := by
        have he : (1:ℝ) + ν * q / (α * q + ν) = (α * q + ν + ν * q) / (α * q + ν) := by
          field_simp
        rw [he, div_le_div_iff₀ hden_r hden_r]
        nlinarith [mul_le_mul_of_nonneg_right hr1lb2 hden_r.le]
      have step2 : 1 + ν * q1 α β ν / (α * q1 α β ν + ν) ≤ 1 + ν * q / (α * q + ν) := by
        have : ν * q1 α β ν / (α * q1 α β ν + ν) ≤ ν * q / (α * q + ν) := by
          rw [div_le_div_iff₀ hden_l hden_r]
          nlinarith [mul_nonneg (mul_nonneg hν.le hν.le)
            (by linarith [hqq1] : (0:ℝ) ≤ q - q1 α β ν)]
        linarith
      rw [hs, hr2v, hμ4c]
      exact le_trans step2 step
  · -- q ≥ 1 : bound below by μ₂
    refine le_trans (min_le_left _ _) ?_
    have hr2C : r2star α ν p = ν := by rw [hr2, min_eq_right hqge1, mul_one]
    have h1αq : 1 ≤ α * q := by nlinarith [mul_le_mul_of_nonneg_left hqge1 hα.le, h.c1_lo]
    have hr1lbC : min β (α * q) + ν ≤ r1star α β ν q := by
      rw [r1star]; refine le_trans (le_of_eq ?_) (le_max_right _ _)
      rw [min_eq_left h1αq, mul_one]
    have hαqpos : 0 < α * q := mul_pos hα hqpos
    have hmn : 0 < min β (α * q) + ν := by
      have : 0 < min β (α * q) := lt_min hβ hαqpos; linarith
    have hsqrt : Real.sqrt (L2 α β ν) ≤ β + ν := by
      rw [show β + ν = Real.sqrt ((β + ν) ^ 2) from (Real.sqrt_sq (by linarith)).symm]
      apply Real.sqrt_le_sqrt
      rw [L2]
      nlinarith [mul_nonneg (by linarith [h.c1_mid] : (0:ℝ) ≤ β - α) hβ.le,
        mul_nonneg hν.le (by linarith [h.c1_mid, h.c1_lo] : (0:ℝ) ≤ 2 * β - α)]
    have hμ2bound : μ2 α β ν ≤ (β + 2 * ν) / (β + ν) := by
      rw [μ2, one_div, q2, inv_div, div_le_div_iff₀ (by linarith) (by linarith)]
      nlinarith [mul_nonneg (by linarith [hsqrt] : (0:ℝ) ≤ β + ν - Real.sqrt (L2 α β ν))
        (by linarith : (0:ℝ) ≤ β + ν)]
    rw [hr2C, hsSum]
    calc μ2 α β ν ≤ (β + 2 * ν) / (β + ν) := hμ2bound
      _ ≤ (min β (α * q) + 2 * ν) / (min β (α * q) + ν) := by
          rw [div_le_div_iff₀ (by linarith) hmn]
          nlinarith [mul_nonneg hν.le (by linarith [min_le_left β (α * q)] :
            (0:ℝ) ≤ β - min β (α * q))]
      _ ≤ (r1star α β ν q + ν) / (min β (α * q) + ν) := by
...
public theorem thm_4 (h : Constraints α β ν) {p q : ℝ}
    (hpaq : p = α * q) (hpq1 : 1 < p + q) :
    min (μ2 α β ν) (μ4 α β ν) ≤ μ α β ν p q := by
