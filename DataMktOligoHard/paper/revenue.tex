...
\begin{align*}
r^-_1(p, q) &= \min(p, \beta) + n \cdot \min(p, \max(0, 1-q))
& r^+_1(p, q) &= \min(p, \beta) + n \cdot \min(p, 1)
\\ r^-_2(p, q) &= n \cdot \min(q, \max(0, 1-p))
& r^+_2(p, q) &= n \cdot \min(q, 1)
\end{align*}
\end{observation}
...
\item If $p + q \ge 1$, then $r^-_1(p, q) + r^+_2(p, q) = r^+_1(p, q) + r^-_2(p, q) = \min(p, \beta) + n$.
\end{enumerate}
\end{observation}
...
\item If $p = \alpha q$ and $p + q > 1$, then $r^-_1(p, q) \le r_1 \le r^+_1(p, q)$,
    $r^-(p, q) \le r_2 \le r^+_2(p, q)$, and $r_1 + r_2 = \min(p, \beta) + n$.
\end{tightenum}
\end{observation}
...
r_1^*(q) &= \max\left(
    \beta + n\max(0, 1-q),
    \min(\beta, \alpha q) + n\min(1, \alpha q)
    \right),
\\ r_2^*(p) &= n\max(1-p, \min(1, p/\alpha)).
\end{align*}
\end{lemma}
